...
class Cluster():
	...
	def addRouter(self, name=None, dpid=None, **kwargs):
		...
		router = self.network.addSwitch(self.name+name, dpid=dpid, cls=CustomBridge, **kwargs)
		'''
		...
		self.mininet.addLink(self.centralSwitch, router, intfName2=router.name+'-eth0')
		...
		self.mininet.addLink(greRouter, router, intfName1=greRouter.name+'-eth0', intfName2=router.name+'-eth1', params1={ 'ip' : '1.1.1.1/24' })
...
class CDN():
	...
	def addBorderNetwork( self, networkId=None):
		...
		bridge = self.border_network.addSwitch(bridgeName, CustomBridge )
	...
	def addCluster( self, controller=None ):
		...
		self.mininet.addLink(self.os3, router, intfName2=router.name+'-eth2')
	...
	def linkBridgeCluster(self, bridge, cluster):
		...
		self.mininet.addLink(bridge, bridgeGre, intfName1=bridge.name+'-'+greNameBridge, intfName2=bridgeGre.name+'-eth0', params2={ 'ip' : '1.1.1.254/24' })
...
class VCDN( Mininet ):
	...
	def construct(self):
		...
		borderController = self.addController('borderController', CustomRyu, path='./RyuControllers/bridge_controller.py')
		...
		cluster1Controller = self.addController('cluster1Controller', CustomRyu, path='./RyuControllers/cluster_controller.py')
		...
		# Controller for cluster1 control the default router
		#	( Controller -> RyuController)
		cluster2Controller = self.addController('cluster2Controller', CustomRyu, path='./RyuControllers/cluster_controller.py')
