...
class DNat(app_manager.RyuApp):
	...
	def __init__(self, *args, **kwargs):
		...
		self.siteorigin_sitecache = {'10.0.20.1': '192.168.0.1', "10.0.20.2": '192.168.0.2', '100.0.0.1': '192.168.0.20'}
		self.mac_to_port = {}
		self.ip_to_mac = {}
	...
	def config_address_port(self, adr_tun='1.1.1.254', adr_int='192.168.0.254', adr_gateway='30.0.0.254', adr_ext='30.0.0.101'):
	...
	def _packet_in_handler(self, ev):
		...
		if pkt_ipv4:
			print "IPV4"
			# -----------------------IF Packet arrive at Tunnel port -----------------------------------
			if in_port == self.PORT_TUN:
				print "Port tunnel"
				
				# ------------------IF server origine managed by cluster---------------
				...
				if cache_dst in self.ip_to_mac:
					...
					datapath.send_msg(out)
					'''
					# ------------- If server not in cluster (redirect to GW) --------------
			...
			# -----------------------IF Packet arrive at Cluster port (connection init in cluster-----------------------------------
			if in_port == self.PORT_INT:
				# Sending ARP request to gateway
				######
				_tcp = pkt.get_protocol(tcp.tcp)
				_udp = pkt.get_protocol(udp.udp)
			...
			'''
			# -----------------------IF Packet arrive at GW port -----------------------------------	
			if in_port == self.PORT_EXT:
				if _arp:
					if _arp.opcode == arp.ARP_REQUEST:
						arp_reply_pkt = self._arp_request_handler(_arp)
						self._send_packet_to_port(datapath, in_port, arp_reply_pkt)
